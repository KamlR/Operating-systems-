...
int main(int argc, char *argv[]) {
    ...
    printMessage(ready_message, sizeof(ready_message));
    // Получаем сообщение от инфо клиента о готовности обмена данными.
    recvfrom(socket_fd, ready_message, sizeof(ready_message), MSG_WAITALL, (const struct sockaddr *) &info_client, &len);
    printMessage(ready_message, sizeof(ready_message));
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        sendto(socket_fd, message_first, sizeof(message_first), MSG_CONFIRM, (const struct sockaddr *) &info_client, len);
        ...
        sendto(socket_fd, message_second, sizeof(message_second), MSG_CONFIRM, (const struct sockaddr *) &info_client, len);
        sleep(2);
    }
}
