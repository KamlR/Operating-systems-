...
// Если первый элемент массива - с, то первый садовник должен быть отключён.
// Если первый элемент массива - s, то сервер завершил свою работу и садовник отключается.
void checkMessage(const char *flowers) {
    if (flowers[0] == 'c') {
        printf("First gardener has stopped working\n");
        exit(0);
    }
    if(flowers[0] == 's'){
        printf("First gardener has stopped working because of server problems\n");
        exit(0);
    }
}
